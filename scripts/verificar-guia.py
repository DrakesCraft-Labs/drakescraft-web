...
W = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parents[2]
...
sf = (WEB / "guia-slimefun.html").read_text(encoding="utf-8")
if plants.exists():
    ...
    reales = {(limpio(m.group(2)), limpio(m.group(3)), limpio(m.group(1)))
              for m in pat.finditer(plants.read_text(encoding="utf-8"))}
...
PROTECCIONES = {"Thor": 601, "Anubis": 721, "Poseidón": 841, "Titán Japeto": 1001,
                ...
                "Titán Caos": 2501}
rangos = (WEB / "guia-rangos.html").read_text(encoding="utf-8")
...
try:
    import yaml
    cfg = yaml.safe_load((W / "Odysseia/src/main/resources/config.yml").read_text(encoding="utf-8"))
    pur = yaml.safe_load((W / "Odysseia/src/main/resources/purchases.yml").read_text(encoding="utf-8"))
    kits = set(cfg["kits"])
...
guia = (WEB / "guia.html").read_text(encoding="utf-8")
...
try:
    boss_cfg = yaml.safe_load((W / "DrakesBosses/src/main/resources/config.yml").read_text(encoding="utf-8"))
